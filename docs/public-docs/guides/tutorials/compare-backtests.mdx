...
- Condition quadrant chart — plots each backtest by two metrics at once (for
  example return vs. drawdown), so stronger and weaker runs land in different
  corners

### Time Periods

- Monthly and weekly return comparison
- Seasonality analysis — whether results tend to repeat at certain times, such
  as a recurring strong month
