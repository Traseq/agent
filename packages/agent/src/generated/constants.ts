// Auto-generated by scripts/sync-constants.ts — do NOT edit manually.
export const DOMAIN_CONSTANTS = {
  MARKET_FIELDS: [
    'open',
    'high',
    'low',
    'close',
    'hl2',
    'hlc3',
    'ohlc4',
    'typical',
    'median',
    'volume',
  ],
  STATE_FIELDS: [
    'position_exists',
    'position_unrealized_pnl',
    'position_realized_pnl',
    'position_bars_since_entry',
    'position_bars_since_exit',
    'account_balance',
    'account_equity',
    'account_gross_exposure',
    'account_used_margin',
    'account_available_collateral',
    'account_margin_utilization',
    'account_drawdown',
    'account_max_drawdown',
    'trades_count_total',
    'trades_count_today',
    'wins_streak',
    'losses_streak',
    'last_trade_pnl',
    'last_entry_price',
    'last_exit_price',
  ],
  PATTERNS: [
    'doji',
    'hammer',
    'shooting_star',
    'bullish_engulfing',
    'bearish_engulfing',
    'morning_star',
    'evening_star',
    'three_white_soldiers',
    'three_black_crows',
    'bullish_harami',
    'bearish_harami',
    'piercing_line',
    'dark_cloud_cover',
    'abandoned_baby',
    'dragonfly_doji',
    'gravestone_doji',
    'tweezers_top',
    'tweezers_bottom',
    'inside_bar',
  ],
  TIMEFRAMES: ['15m', '1h', '4h', '1d'],
  WEEKDAYS: ['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'],
  ENTRY_SIZING_MODES: [
    'fixed',
    'fixed_cash',
    'percent_equity',
    'percent_balance',
  ],
  EXIT_SIZING_MODES: ['fixed', 'percent_position'],
  SIDES: ['long', 'short'],
  COMPARE_OPS: ['gt', 'lt', 'gte', 'lte', 'eq', 'neq'],
  CROSS_OPS: ['cross_up', 'cross_down'],
  TOLERANCE_MODES: ['absolute', 'percent'],
  ROLLING_OPERATORS: ['max', 'min', 'sum', 'avg'],
  ROLLING_WINDOW_MODES: [
    'any',
    'none',
    'all',
    'streak',
    'count_gt',
    'count_lt',
    'count_eq',
  ],
  MATH_OPERATORS: [
    'add',
    'sub',
    'mul',
    'div',
    'abs',
    'min',
    'max',
    'pow',
    'log',
    'sqrt',
  ],
  CONFLICT_POLICIES: ['set_priority', 'reset_priority'],
  PIVOT_KINDS: ['high', 'low'],
  SOURCE_EDITORS: [
    'token-flow',
    'text-dsl',
    'ai',
    'runtime-strategy',
    'signal-graph',
  ],
} as const;
