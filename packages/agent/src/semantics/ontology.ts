...
export const SEMANTIC_FACETS: readonly SemanticFacetDefinition[] = [
  ...
  facet(
    ...
    'trend',
    'context_filter',
    'SuperTrend trend_direction output identifies bullish or bearish trend regime.',
    ['supertrend', 'supertrend direction', 'trend direction'],
    ['trend.supertrend_bullish_regime', 'trend.supertrend_bearish_regime'],
  ),
  facet(
    'trend.supertrend_cross',
    'trend',
    'entry_trigger',
    'Close crossing the SuperTrend line marks a directional trend flip.',
    ['supertrend cross', 'cross supertrend', 'supertrend flip'],
    ['trend.close_crosses_up_supertrend'],
  ),
  facet(
    'trend.supertrend_exit',
    'trend',
    'exit',
    'Close crossing down through the SuperTrend line exits a long trend-following position.',
    ['supertrend exit', 'cross down supertrend', 'supertrend bearish flip'],
    ['trend.close_crosses_down_supertrend'],
  ),
  facet(
...
export const SEMANTIC_IMPLEMENTATIONS: readonly SemanticImplementationDefinition[] =
  [
    ...
    implementation({
      id: 'trend.supertrend_bullish_regime',
      semanticIds: ['trend.supertrend_direction'],
      role: 'context_filter',
      description: 'SuperTrend trend_direction output is bullish (+1).',
      complexity: 'simple',
      curatedPriority: 88,
      requiredCapabilities: {
        nodeKinds: ['indicator', 'compare'],
      ...
      fragment: {
        nodes: [
          ...
          },
          {
            ...
            kind: 'compare',
            op: 'eq',
            left: { ref: 'supertrend_direction' },
            right: { const: 1 },
          },
        ],
        assemblyHints: { contextFilters: [{ ref: 'supertrend_bullish' }] },
        settingsHints: { warmupPeriod: 50 },
      },
      tradeoffs: {
        strengths: ['Uses SuperTrend direction directly as a regime filter.'],
        risks: ['Can lag during sharp reversals.'],
        assumptions: ['ATR length 10 and multiplier 3 are standard defaults.'],
      },
      validationHints: [
        'SuperTrend requires top-level output "trend_direction" in SignalGraph.',
      ],
    }),
    implementation({
      id: 'trend.supertrend_bearish_regime',
      semanticIds: ['trend.supertrend_direction'],
      role: 'context_filter',
      description: 'SuperTrend trend_direction output is bearish (-1).',
      complexity: 'simple',
      curatedPriority: 72,
      requiredCapabilities: {
        nodeKinds: ['indicator', 'compare'],
      ...
      fragment: {
        ...
        assemblyHints: { contextFilters: [{ ref: 'supertrend_bearish' }] },
        settingsHints: { warmupPeriod: 50 },
      },
      tradeoffs: {
        strengths: ['Directly expresses bearish SuperTrend regime.'],
        risks: ['Usually needs side-specific entry handling for short use.'],
        assumptions: ['ATR length 10 and multiplier 3 are standard defaults.'],
      },
      validationHints: [
        'Use with short-side strategies or as a long-entry blocker.',
      ],
    }),
    implementation({
      id: 'trend.close_crosses_up_supertrend',
      semanticIds: ['trend.supertrend_cross'],
      role: 'entry_trigger',
      description: 'Close crosses above the SuperTrend line.',
      complexity: 'simple',
      curatedPriority: 86,
      requiredCapabilities: {
        nodeKinds: ['market', 'indicator', 'cross'],
        indicators: ['supertrend'],
        operators: ['cross_up'],
      },
      fragment: {
        nodes: [
          ...
          },
          {
            ...
            op: 'cross_up',
            left: { ref: 'close_price' },
            right: { ref: 'supertrend_line' },
          },
        ],
        assemblyHints: { entryTrigger: { ref: 'supertrend_bullish_flip' } },
        settingsHints: { warmupPeriod: 50 },
      },
      tradeoffs: {
        strengths: ['Captures a familiar SuperTrend bullish flip.'],
        risks: ['Can whipsaw in sideways markets without a trend filter.'],
        assumptions: ['ATR length 10 and multiplier 3 are standard defaults.'],
      },
      validationHints: [
        'SuperTrend line uses top-level output "supertrend" in SignalGraph.',
      ],
    }),
    implementation({
      id: 'trend.close_crosses_down_supertrend',
      semanticIds: ['trend.supertrend_exit'],
      role: 'exit',
      description: 'Close crosses below the SuperTrend line.',
      complexity: 'simple',
      curatedPriority: 80,
      requiredCapabilities: {
        nodeKinds: ['market', 'indicator', 'cross'],
      ...
      fragment: {
        ...
        assemblyHints: { signalExit: { ref: 'supertrend_bearish_flip' } },
        settingsHints: { warmupPeriod: 50 },
      },
      tradeoffs: {
        strengths: ['Provides a trend-following exit tied to SuperTrend.'],
        risks: ['May exit late after a fast drop.'],
        assumptions: ['ATR length 10 and multiplier 3 are standard defaults.'],
      },
      validationHints: ['Use as an exit for long SuperTrend strategies.'],
    }),
    implementation({
